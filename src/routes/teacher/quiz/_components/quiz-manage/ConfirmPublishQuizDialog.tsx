...
export function ConfirmPublishQuizDialog({
  ...
  onNavigateSetting?: (target: string, focus: boolean) => void;
}) {
  ...
  return (
    <ConfirmDialog
      ...
      description={t(
        `teacher_quiz_manage.confirm_publish.${tab === "preview" ? "body_confirm" : "body_preview"}`,
        { count: approvedCount },
      )}
      confirmLabel={t("teacher_quiz_manage.actions.publish")}
      ...
      extraContent={
        <div className="space-y-3">
          <div className="max-h-[50vh] overflow-y-auto">
            <SettingsSummary draft={draft} onNavigate={onNavigateSetting} />
          </div>
          {tab !== "preview" && (
            <Button
              type="button"
