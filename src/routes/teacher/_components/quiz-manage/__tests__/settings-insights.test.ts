...
describe("quiz settings insights", () => {
  ...
  it("disables hidden SM-2 settings in the payload when spaced repetition is off", () => {
    const payload = settingsPatchFromDraft({
      ...settingsFixture(),
    ...
    expect(payload).toMatchObject({
      feeds_spaced_repetition: false,
      initial_ef: null,
      min_ef_for_unlock: null,
      coverage_threshold: null,
      reminders_enabled: false,
    });
  });
  it.each(["2026-10-01T10:00", "2026-10-01T09:00"])(
    "rejects close %s at or before opening",
    (until) => {
      expect(
        settingsErrors({
          ...settingsFixture(),
          available_from: "2026-10-01T10:00",
          available_until: until,
        }),
      ).toContain("close_before_open");
    },
  );
  it("accepts open-ended and valid schedules", () => {
    ...
    expect(
      settingsErrors({
        ...settingsFixture(),
        available_from: "2026-10-01T10:00",
        available_until: "2026-10-01T11:00",
      }),
    ).toEqual([]);
  });
  it.each([NaN, -1, 101])("rejects invalid passing score %s", (score) => {
    expect(
      settingsErrors({ ...settingsFixture(), passing_score_percent: score }),
    ).toContain("invalid_score");
  });
  it("warns about delayed review without a close date, even when due date exists", () => {
    expect(
      settingsWarnings({
        ...settingsFixture(),
        review_options: PRESETS.nothing_until_close(),
        due_at: "2026-10-01T10:00",
      }),
    ).toContain("review_needs_close");
  });
  it("does not warn for identical review windows or a configured close", () => {
    expect(settingsWarnings(settingsFixture())).not.toContain(
      "review_needs_close",
    );
    expect(
      settingsWarnings({
        ...settingsFixture(),
        review_options: PRESETS.nothing_until_close(),
        available_until: "2026-10-01T10:00",
      }),
    ).not.toContain("review_needs_close");
  });
  it("makes an out-of-window due date a warning, not a new business restriction", () => {
    const draft = {
      ...settingsFixture(),
      available_until: "2026-10-01T10:00",
      due_at: "2026-10-02T10:00",
    };
  ...
  it("only exposes multi-attempt grading when more than one attempt is possible", () => {
    expect(
      hasMultipleAttempts({ allow_retakes: false, max_attempts: "" }),
    ).toBe(false);
    expect(
      hasMultipleAttempts({ allow_retakes: true, max_attempts: "1" }),
    ).toBe(false);
    expect(hasMultipleAttempts({ allow_retakes: true, max_attempts: "" })).toBe(
      true,
    );
    expect(
      hasMultipleAttempts({ allow_retakes: true, max_attempts: "3" }),
    ).toBe(true);
  });
});
