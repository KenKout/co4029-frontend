...
export function GradebookTab({
  ...
  const rows = useMemo(() => (data ?? []).filter((row) => {
    ...
    const matchesSearch = !needle || `${row.student_name ?? ""} ${row.student_email ?? ""} ${row.student_id}`.toLowerCase().includes(needle);
    const matchesStatus = status === "all" || (status === "passed" && row.passed) || (status === "failed" && !row.passed);
  ...
  const columns: DataTableColumn<QuizGradeRow>[] = [
    ...
    { id: "method", header: t("teacher_quiz_results.gradebook.col_method"), cell: (row) => row.grading_method },
    { id: "attempts", header: t("teacher_quiz_results.gradebook.col_attempts"), align: "right", sortable: true, sortValue: (row) => row.attempts_counted, cell: (row) => <span className="tabular-nums">{row.attempts_counted}</span> },
  ];
      ...
      <DataTableToolbar
        ...
        trailing={
          <div className="flex items-center gap-1">
            <Button variant="outline" size="sm" className="gap-1.5" disabled={downloading} onClick={() => onDownload("csv")}><Download className="h-4 w-4" />CSV</Button>
            <Button variant="outline" size="sm" className="gap-1.5" disabled={downloading} onClick={() => onDownload("xlsx")}><Download className="h-4 w-4" />XLSX</Button>
          </div>
        }
      />
