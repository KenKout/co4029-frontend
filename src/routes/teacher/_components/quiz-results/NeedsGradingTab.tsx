...
export function NeedsGradingTab({ quizId }: { quizId: string }) {
  ...
  const columns: DataTableColumn<NeedsGradingRow>[] = [
    ...
    {
      id: "answer",
      header: t("teacher_quiz_results.grading.col_answer"),
      cell: (row) => (
        <span
          className="block max-w-sm truncate"
          title={row.answer_text ?? undefined}
        >
    ...
    {
      id: "grade",
      header: t("teacher_quiz_results.grading.col_action"),
...
function GradeActionCell({
  ...
  return (
    <div className="flex min-w-[360px] items-end gap-2">
      <Input
        size="md"
        aria-label={t("teacher_quiz_results.grading.score_label")}
        type="number"
        min={0}
        step="0.25"
        value={score}
      ...
      <Input
        size="md"
        aria-label={t("teacher_quiz_results.grading.feedback_label")}
        value={feedback}
